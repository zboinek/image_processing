...
df_normal_copy[["img_name", "extension"]] = df_normal_copy[
    "X_ray_image_name"
...
df_normal_copy2[["img_name", "extension"]] = df_normal_copy2[
    "X_ray_image_name"
].str.rsplit(".", n=1, expand=True)
...
df_normal_copy3[["img_name", "extension"]] = df_normal_copy3[
    "X_ray_image_name"
].str.rsplit(".", n=1, expand=True)
...
start = time.time()
p = Pool()
normal_images2 = p.map(
    lambda filename: cv2.imread(
        os.path.join(TRAIN_DIR, filename), cv2.IMREAD_UNCHANGED
    ),
    df_normal["X_ray_image_name"],
)
p.close()
p.join()
end = time.time()
